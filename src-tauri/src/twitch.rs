...
impl TwitchValidator {
    ...
    pub fn extract_channel_name(url: &str) -> Option<String> {
        ...
        if trimmed.contains("/dashboard") {
            ...
            let dashboard_regex =
                Regex::new(r"^https?://(www\.)?twitch\.tv/([^/]+)/dashboard").ok()?;
...
mod tests {
    ...
    fn test_valid_urls() {
        ...
        assert!(TwitchValidator::is_valid_url(
            "https://www.twitch.tv/summit1g/dashboard"
        ));
        assert!(TwitchValidator::is_valid_url(
            "https://twitch.tv/shroud/dashboard"
        ));
        ...
        assert!(TwitchValidator::is_valid_url(
            "twitch.tv/abcdefghijklmnopqrstuvwxy"
        ));
    }
    ...
    fn test_invalid_urls() {
        ...
        assert!(!TwitchValidator::is_valid_url(
            "twitch.tv/abcdefghijklmnopqrstuvwxyz"
        ));
        ...
        // Wrong domain
        assert!(!TwitchValidator::is_valid_url(
            "https://youtube.com/summit1g"
        ));
        assert!(!TwitchValidator::is_valid_url(
            "https://twitch.com/summit1g"
        ));
    ...
    fn test_channel_extraction() {
        // Full URLs
        assert_eq!(
            TwitchValidator::extract_channel_name("https://www.twitch.tv/summit1g"),
            Some("summit1g".to_string())
        );
        assert_eq!(
            TwitchValidator::extract_channel_name("https://twitch.tv/shroud"),
            Some("shroud".to_string())
        );
        assert_eq!(
            TwitchValidator::extract_channel_name("http://www.twitch.tv/pokimane/"),
            Some("pokimane".to_string())
        );

        // Partial URLs
        assert_eq!(
            TwitchValidator::extract_channel_name("www.twitch.tv/xqc"),
            Some("xqc".to_string())
        );
        assert_eq!(
            TwitchValidator::extract_channel_name("twitch.tv/ninja"),
            Some("ninja".to_string())
        );
        assert_eq!(
            TwitchValidator::extract_channel_name("twitch.tv/tfue/"),
            Some("tfue".to_string())
        );

        // Just usernames
        assert_eq!(
            TwitchValidator::extract_channel_name("summit1g"),
            Some("summit1g".to_string())
        );
        assert_eq!(
            TwitchValidator::extract_channel_name("shroud"),
            Some("shroud".to_string())
        );
        ...
        assert_eq!(
            TwitchValidator::extract_channel_name("https://youtube.com/summit1g"),
            None
        );
